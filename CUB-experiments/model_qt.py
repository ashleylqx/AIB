...
def xavier_init(ms):
    if isinstance(ms, NearestEmbed) or isinstance(ms, NearestEmbedEMA) or isinstance(ms, VectorQuantizer):
        return
    for m in ms:
...
class VaNet_vgg(nn.Module):
    ...
    def __init__(self, K=K_dim, att_K=A_dim, qt_num=2, vq_coef=0.2, comit_coef=0.4, rd_init=False,
                 decay=None, qt_trainable=False, backbone='vgg', return_att=False):
        ...
        # wrn_50_2 qt cub_crop
        elif self.backbone == 'wrn_50_2':
            '''------- wrn_50_2 ------'''
            ...
            self.tmp_layer = self._make_layer(block, 256, layers[2])  # , stride=2, dilate=False
            self.att_layer4 = self._make_layer(block, 512, layers[3])  # , stride=2, dilate=True
            ...
            self.att_module_mu = nn.Sequential(
                # self.att_layer3,
                self.att_layer4,
                nn.Conv2d(self.channel, 1, 3, stride=1, padding=1),  # f11 original
                nn.Sigmoid()  # sigmu
            )
    ...
    # f4 f7 f11
    def forward(self, x, num_sample=1, train=True, hd=False):
        ...
        # att_mask_q, _ = self.emb(att_mask_flt, weight_sg=True)  # same dim as att_mask_flt
        # att_mask_emb, _ = self.emb(att_mask_flt.detach())
        ...
        # att_mask_emb = att_mask_emb.view(num_sample, x.size(0), self.att_dim * self.att_dim)
        ...
        if train:
            ...
            return logit, logit, latent_loss, latent_loss, vq_loss, commit_loss
            ...
            return logit, logit, att_mask, att_mask_q, att_mask_q
            # return logit, logit_ori, att_mask, att_mask_q, att_mask_emb
