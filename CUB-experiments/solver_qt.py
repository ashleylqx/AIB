...
from model_qt import VaNet_vgg, VaNet_wrn
...
class SolverVA(object):
    ...
    def __init__(self, args):
        ...
        # Quantum embedding
        self.qt_num = args.qt_num
        self.vq_coef = args.vq_coef
        self.comit_coef = args.comit_coef
        self.rd_init = args.rd_init
        self.decay = args.decay
        self.qt_trainable = args.qt_trainable
        ...
        if self.backbone == 'vgg16':
            ...
            VaNet = VaNet_wrn
        ''''''
        self.model = cuda(VaNet(self.K, self.att_K, self.qt_num, self.vq_coef, self.comit_coef,
                                              self.rd_init, self.decay, self.qt_trainable, backbone=self.backbone), self.cuda)
        ...
        if torch.cuda.device_count() > 1:
            ...
            self.model_ema = Weight_EMA_Update(torch.nn.DataParallel(
                cuda(VaNet(self.K, self.att_K, self.qt_num, self.vq_coef, self.comit_coef,
                                         self.rd_init, self.decay, self.qt_trainable, backbone=self.backbone), self.cuda)), \
        ...
        self.history['vq_loss']=0.
        self.history['commit_loss']=0.
        self.history['qt_loss']=0.
    ...
    def train_anneal(self):
        ...
        for e in range(self.epoch):
            ...
            for idx, (images, labels) in enumerate(self.data_loader['train']):
                ...
                logit, logit_ori, latent_loss1, latent_loss2, vq_loss, commit_loss = self.model(x, self.num_sample, hd=True)
                ...
                qt_loss = self.vq_coef * vq_loss + self.comit_coef * commit_loss
                # total_loss = class_loss + self.beta * info_loss
                total_loss = class_loss + beta_anneal * info_loss + qt_loss
                # if self.min_zax:
                ...
                if self.num_avg != 0:
                    avg_soft_logit, avg_soft_logit_ori, _, _, _, _ = self.model(x, self.num_avg)
                ...
                if self.global_iter % 150 == 0:
                    ...
                    print('latent1:{:.2f} latent2:{:.2f} vq:{:.2f} comit:{:.2f}'
                          .format(latent_loss1.mean().data.item(), latent_loss2.mean().data.item(), vq_loss.mean().data.item(),
                                  commit_loss.mean().data.item()), end=' ')
                    # if self.min_zax:
                    #     print('IAX:{:.2f}'
                    ...
                    if self.tensorboard:
                        ...
                        self.tf.add_scalars(main_tag='performance/cost',
                                            tag_scalar_dict={
                                                ...
                                                'train_one-shot_vq': vq_loss.mean().data.item(),
                                                'train_one-shot_commit': commit_loss.mean().data.item(),
                                                'train_one-shot_qt': qt_loss.mean().data.item(),
                        ...
                        if torch.cuda.device_count() > 1:
                            self.tf.add_scalars(main_tag='grad/train',
                                                tag_scalar_dict={
                                                    ...
                                                    # 'att_mask_post': self.model.module.att_module_post[-1].weight.grad.abs().mean().item(),
                                                    'att_mask': self.model.module.att_module_mu[0].weight.grad.abs().mean().item(),
                                                    # 'att_mask_post': self.model.module.att_module_mu_post[0].weight.grad.abs().mean().item(),
                            ...
                            self.tf.add_scalars(main_tag='grad/train',
                                                tag_scalar_dict={
                                                    ...
                                                    # 'att_mask_post': self.model.att_module_post[-1].weight.grad.abs().mean().item(),
                                                    'att_mask': self.model.att_module_mu[0].weight.grad.abs().mean().item(),
                                                    # 'att_mask_post': self.model.att_module_mu_post[0].weight.grad.abs().mean().item(),
                                                    'encode': self.model.encode[-1].weight.grad.abs().mean().item(),
                                                    'decode': self.model.decode[-1].weight.grad.abs().mean().item()},
                                                global_step=self.global_iter)

                        if self.decay is None and self.qt_trainable:
                            if torch.cuda.device_count() > 1:
                                self.tf.add_scalars(main_tag='grad/train_qt',
                                                    tag_scalar_dict={
                                                        'qt_weight': self.model.module.emb.weight.grad.abs().mean().item()},
                                                    global_step=self.global_iter)
                            else:
                                self.tf.add_scalars(main_tag='grad/train_qt',
                                                    tag_scalar_dict={
                                                        'qt_weight': self.model.emb.weight.grad.abs().mean().item()},
                                                    global_step=self.global_iter)
    ...
    def train(self):
        ...
        for e in range(self.epoch):
            ...
            # for idx, (images, labels) in enumerate(self.data_loader['train']):
                ...
                # info_loss = 0.3333 * (latent_loss1 + latent_loss2 + 0.01*latent_loss3).mean().div(math.log(2)) #div
                info_loss = 0.5 * (latent_loss1 + latent_loss2 ).mean().div(math.log(2)) #div
                # info_loss = (latent_loss1 + latent_loss2).mean()
                # if self.min_zax:
                #     info_loss += -0.5 * (1 + 2 * att_std.log() - att_mu.pow(2) - att_std.pow(2)).sum(1).mean().div(math.log(2))
                qt_loss = self.vq_coef * vq_loss + self.comit_coef * commit_loss
                total_loss = class_loss + self.beta * info_loss + qt_loss.mean() # one
                ...
                #           .format(e, idx, int(N), izy_bound.data.item(), izx_bound.data.item()), end=' ')
                #     print('latent1:{:.2f} latent2:{:.2f} vq:{:.2f} comit:{:.2f}'
                #           .format(latent_loss1.mean().data.item(), latent_loss2.mean().data.item(),
                #                   vq_loss.mean().data.item(), commit_loss.mean().data.item()), end=' ')
                #     # if self.min_zax:
                #     #     print('IAX:{:.2f}'
                #     #           .format(info_att_loss.data.item()), end=' ')
                #     print('acc:{:.4f} avg_acc:{:.4f}'
    ...
    def test(self, save_ckpt=False, save_att=False):
        ...
        if hasattr(self.model, 'emb'):
            print(self.model.emb.weight.data)
        else:
            print(self.model.module.emb.weight.data)
        for idx, (images, labels) in enumerate(self.data_loader['test']):

            x = Variable(cuda(images, self.cuda))
            y = Variable(cuda(labels, self.cuda))
            # y_one_hot = F.one_hot(y, n_class).type_as(x)
            logit, logit_ori, l_loss1, l_loss2, v_loss, cmt_loss = self.model(x, self.num_sample)
            #pdb.set_trace()
            ...
            vq_loss += current_num * 1.0 / total_num * (v_loss.mean().item() - vq_loss)
            commit_loss += current_num * 1.0 / total_num * (cmt_loss.mean().item() - commit_loss)
        ...
        qt_loss = self.vq_coef * vq_loss + self.comit_coef * commit_loss
        total_loss = class_loss + self.beta * info_loss + qt_loss # one
        # total_loss = class_loss + class_loss_ori + self.beta * info_loss  # two
        # total_loss = 0.5 * (class_loss + class_loss_ori) + self.beta * info_loss  # two_2
        ...
        print('latent1:{:.2f} latent2:{:.2f} vq:{:.2f} comit:{:.2f}'
              .format(latent_loss1, latent_loss2, vq_loss, commit_loss), end=' ')
        #if self.min_zax:
        #    print('IAX:{:.2f}'
        ...
        if self.history['avg_acc'] < avg_accuracy and self.history['avg_acc_ema'] < avg_accuracy:
            ...
            self.history['vq_loss'] = vq_loss
            self.history['commit_loss'] = commit_loss
            self.history['qt_loss'] = qt_loss
            #if self.min_zax:
        ...
        if self.tensorboard:
            ...
            self.tf.add_scalars(main_tag='performance/cost',
                                tag_scalar_dict={
                                    ...
                                    'test_one-shot_vq': vq_loss,
                                    'test_one-shot_commit': commit_loss,
                                    'test_one-shot_qt': qt_loss,
    ...
    def test_ema(self, save_ckpt=False, save_att=False):
        ...
        for idx, (images, labels) in enumerate(self.data_loader['test']):
            ...
            y = Variable(cuda(labels, self.cuda))
            # y_one_hot = F.one_hot(y, n_class).type_as(x)
            logit, logit_ori, l_loss1, l_loss2, v_loss, cmt_loss = self.model_ema.model(x, self.num_sample)
            #pdb.set_trace()
            current_num = y.size(0)
            total_num += current_num
    ...
    def save_attention(self):
        ...
        for idx, (images, labels) in enumerate(self.data_loader['test']):
            ...
            y = Variable(cuda(labels, self.cuda))
            # y_one_hot = F.one_hot(y, n_class).type_as(x)
            pred_logits, pred_logits_ori, att_maps, att_maps_q, att_maps_emb = self.model(x, self.num_sample, train=False)
            ...
            att_maps_q_list = []
            for d_i in range(torch.cuda.device_count()):
                att_maps_q_list.append(att_maps_q[d_i * self.num_sample:(d_i + 1) * self.num_sample, :, :])
            att_maps_q = torch.cat(att_maps_q_list, dim=1)

            att_maps_emb_list = []
            for d_i in range(torch.cuda.device_count()):
                att_maps_emb_list.append(att_maps_emb[d_i * self.num_sample:(d_i + 1) * self.num_sample, :, :])
            att_maps_emb = torch.cat(att_maps_emb_list, dim=1)
            ...
            for b_num in range(self.test_batch):
                ...
                for a_num in range(att_maps_q.size(0)):
                    ...
                    scipy.misc.imsave(os.path.join(self.att_dir, 'test_{:04d}_cls{:02d}_{}_{}_att{:02d}_q.png'.format(
                ...
                for a_num in range(att_maps_emb.size(0)):
                    ...
                    tmp_map = cv2.resize(att_map, (ImgSize[1], ImgSize[0]), interpolation=cv2.INTER_NEAREST)
                    # print('max %.4f min %.4f' % (np.max(tmp_map), np.min(tmp_map)))
                    scipy.misc.imsave(os.path.join(self.att_dir, 'test_{:04d}_cls{:02d}_{}_{}_att{:02d}_emb.png'.format(
                        cnt, labels[b_num], flag[b_num], flag_ori[b_num], a_num)), tmp_map.astype('float')*255.)
    ...
    def load_checkpoint(self, filename='best_acc.tar'):
        ...
        if file_path.is_file():
            ...
            if torch.cuda.device_count()>1:
                ...
                print(tmp_state_dict['emb.weight'])
            else:
                ...
                print(checkpoint['model_states']['net']['emb.weight'])
